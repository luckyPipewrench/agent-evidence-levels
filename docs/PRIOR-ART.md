...
| RFC 6962 / RFC 9162 (Certificate Transparency) | ALIGN | The append-only-log, Merkle inclusion and consistency proof, and signed tree head model informs AEL-3. CT proves log inclusion and history consistency, not the truth of the underlying claim, which is exactly AEL's boundary. AEL v0.1 carries and verifies inclusion proofs only; it does not verify history consistency between signed tree heads. |
