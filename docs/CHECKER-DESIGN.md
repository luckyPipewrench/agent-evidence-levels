...
- **q `unanchored`** — records with seq beyond the anchored head labeled `UNANCHORED-WINDOW`.
...
| AEL-3 | AEL-2 + n, o (p/q are adversarial/labeling duties) |
| AEL-4 | AEL-3 + r, s, t |

Sub-dimension → gate: verifier-portability (a not UV) required at every rung, vendor-only ⇒ Ungraded;
chain-continuity gates 0/1; recorder-custody (l) gates 2; external-anchoring (n,o) gates 3;
counterparty-independence (r,s,t) gates 4; mediation-coverage, retention = annotations (bound claims,
do not lower the number); decision-reproducibility = the R suffix.
